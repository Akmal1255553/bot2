...
async def callback_history_page(query: CallbackQuery, session: AsyncSession) -> None:
    ...
    if offset < 0:
        offset = 0
    await _send_history_page(query.message, session, offset=offset, from_user_id=query.from_user.id)
    await query.answer()
...
async def _send_history_page(
    ...
    if total == 0:
        await message.answer(
            ...
            "No images generated yet. Use /generate_image to create your first!"
        )
        return

    entries = await services.history_repo.get_recent(user.id, limit=PAGE_SIZE, offset=offset)

    lines = [f"📜 <b>Generation History</b>  ({offset + 1}–{offset + len(entries)} of {total})\n"]
    for i, entry in enumerate(entries, start=offset + 1):
        lines.append(format_history_entry(entry, i))

    keyboard = history_pagination_keyboard(offset, total, PAGE_SIZE)
